...
export const leavePoliciesRouter = {
    ...
    .handler(async ({ input, context }) => {
      ...
      const [policy] = await db
        ...
        .set({
          ...
          ...(updates.maxConcurrentAbsences !== undefined
            ? { maxConcurrentAbsences: updates.maxConcurrentAbsences }
            : {}),
          ...(updates.maxRequestsPerYear !== undefined
            ? { maxRequestsPerYear: updates.maxRequestsPerYear }
            : {}),
          ...(updates.requiresHrOverrideForSplit !== undefined
            ? { requiresHrOverrideForSplit: updates.requiresHrOverrideForSplit }
            : {}),
  ...
  delete: requireRole("leave_policy", "delete")
    .input(z.object({ id: z.string() }))
    .handler(async ({ input, context }) => {
      const before = await db.query.leavePolicies.findFirst({
        where: eq(leavePolicies.id, input.id),
